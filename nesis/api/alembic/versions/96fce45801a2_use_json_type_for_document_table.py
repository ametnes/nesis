...
def upgrade() -> None:
    ...
    op.add_column('datasource', sa.Column('schedule', sa.Unicode(length=255), nullable=True))
    op.alter_column('document', 'base_uri',
    ...
    op.alter_column('document', 'filename',
               ...
               type_=sa.Unicode(length=4000),
               existing_nullable=False)
    op.alter_column('document', 'rag_metadata',
               existing_type=postgresql.JSONB(astext_type=sa.Text()),
               type_=sa.JSON(),
               existing_nullable=True)
    op.alter_column('document', 'extract_metadata',
               existing_type=postgresql.JSONB(astext_type=sa.Text()),
               type_=sa.JSON(),
               existing_nullable=True)
    op.alter_column('document', 'store_metadata',
               existing_type=postgresql.JSONB(astext_type=sa.Text()),
               type_=sa.JSON(),
               existing_nullable=True)
    op.drop_constraint('uq_document_uuid_base_url_filename', 'document', type_='unique')
    op.create_unique_constraint('uq_document_uuid_datasource_id', 'document', ['uuid', 'datasource_id'])
...
def downgrade() -> None:
    ...
    op.create_unique_constraint('uq_document_uuid_base_url_filename', 'document', ['uuid', 'base_uri', 'filename'])
    op.alter_column('document', 'store_metadata',
